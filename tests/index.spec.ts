...
describe("config function comprehensive tests", () => {
    ...
    beforeEach(() => {
        ...
        loadEnvSpy = vi.spyOn(loadEnvModule, "loadEnvironment").mockImplementation(
        ...
        validateSpy = vi.spyOn(validateModule, "validateSchema").mockImplementation(
            (_schema, _env) => {
                // no-op
            }
        );
    ...
    it("applies escape sequences", async () => {
        loadEnvSpy.mockImplementationOnce(async (env: Map<string, string>, options?: any) => {
